...
#ifdef CONFIG_HYMOFS
struct filename *__original_getname_flags(const char __user *filename, int flags);

/* Hook getname_flags to intercept path lookups */
struct filename *getname_flags(const char __user *filename, int flags)
{
	struct filename *result = __original_getname_flags(filename, flags);
	return hymofs_handle_getname(result);
}
#endif
struct filename *
#ifdef CONFIG_HYMOFS
__original_getname_flags(const char __user *filename, int flags)
#else
